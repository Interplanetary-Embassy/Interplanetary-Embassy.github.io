...
function generatePlanetAsynchronous()
{
    ...
    seed = unicorn_planet.seed
    
	var random = new XorShift128(parseInt(seed));
...
function buildSurfaceRenderObject(tiles, random, action)
{
	...
	action.executeSubaction(function(action)
	{
        ...
        // Check for planet first?
        
        var ocean_color = 'rgb('+unicorn_planet.oceans[0]+'%, '+unicorn_planet.oceans[1]+'%, '+unicorn_planet.oceans[2]+'%)';
        var ocean_color_deep = 'rgb('+(parseInt(unicorn_planet.oceans[0]) + 15)+'%, '+(parseInt(unicorn_planet.oceans[1]) + 15)+'%, '+(parseInt(unicorn_planet.oceans[2]) + 15)+'%)';
        
        var forest_color = 'rgb('+unicorn_planet.rural[0]+'%, '+unicorn_planet.rural[1]+'%, '+unicorn_planet.rural[2]+'%)';
        var forest_color_deep = 'rgb('+(parseInt(unicorn_planet.rural[0]) + 15)+'%, '+(parseInt(unicorn_planet.rural[1]) + 15)+'%, '+(parseInt(unicorn_planet.rural[2]) + 15)+'%)';

        var desert_color = 'rgb('+unicorn_planet.urban[0]+'%, '+unicorn_planet.urban[1]+'%, '+unicorn_planet.urban[2]+'%)';
        var desert_color_deep = 'rgb('+(parseInt(unicorn_planet.urban[0]) + 15)+'%, '+(parseInt(unicorn_planet.urban[1]) + 15)+'%, '+(parseInt(unicorn_planet.urban[2]) + 15)+'%)';
        
		if (tile.elevation <= 0)
		{
			var normalizedElevation = Math.min(-tile.elevation, 1);
			if (tile.biome === "ocean") terrainColor = new THREE.Color(ocean_color).lerp(new THREE.Color(ocean_color_deep), Math.min(-tile.elevation, 1)).lerp(colorDeviance, 0.10);
			else if (tile.biome === "oceanGlacier") terrainColor = new THREE.Color(ocean_color).lerp(colorDeviance, 0.10);
			else terrainColor = new THREE.Color(ocean_color_deep);
		}
		else if (tile.elevation < 0.6)
		{
			var normalizedElevation = tile.elevation / 0.6;
			if (tile.biome === "desert") terrainColor = new THREE.Color(desert_color).lerp(new THREE.Color(desert_color_deep), normalizedElevation).lerp(colorDeviance, 0.10);
            else if (tile.biome === "rainForest") terrainColor = new THREE.Color(forest_color).lerp(new THREE.Color(forest_color_deep), normalizedElevation).lerp(colorDeviance, 0.20);
			else if (tile.biome === "rocky") terrainColor = new THREE.Color(desert_color_deep).lerp(new THREE.Color(desert_color), normalizedElevation).lerp(colorDeviance, 0.15);
			...
			else if (tile.biome === "grassland") terrainColor = new THREE.Color(forest_color_deep).lerp(new THREE.Color(forest_color), normalizedElevation).lerp(colorDeviance, 0.15);
			else if (tile.biome === "swamp") terrainColor = new THREE.Color(desert_color).lerp(new THREE.Color(desert_color_deep), normalizedElevation).lerp(colorDeviance, 0.25);
			else if (tile.biome === "deciduousForest") terrainColor = new THREE.Color(forest_color_deep).lerp(new THREE.Color(forest_color), normalizedElevation).lerp(colorDeviance, 0.10);
			else if (tile.biome === "tundra") terrainColor = new THREE.Color(forest_color).lerp(new THREE.Color(forest_color_deep), normalizedElevation).lerp(colorDeviance, 0.15);
			//else if (tile.biome === "landGlacier") terrainColor = new THREE.Color(0xDDEEFF).lerp(colorDeviance, 0.10);
			else terrainColor = new THREE.Color(forest_color);
		}
		else if (tile.elevation < 0.8)
		{
			var normalizedElevation = (tile.elevation - 0.6) / 0.2;
            
			if (tile.biome === "tundra") terrainColor = new THREE.Color(forest_color).lerp(new THREE.Color(forest_color_deep), normalizedElevation).lerp(colorDeviance, 0.10);
			else if (tile.biome === "coniferForest") terrainColor = new THREE.Color(forest_color_deep).lerp(new THREE.Color(forest_color), normalizedElevation).lerp(colorDeviance, 0.10);
			...
			else if (tile.biome === "mountain") terrainColor = new THREE.Color(desert_color).lerp(new THREE.Color(desert_color_deep), normalizedElevation).lerp(colorDeviance, 0.05);
			else terrainColor = new THREE.Color(forest_color_deep);
		}
		else
		{
			var normalizedElevation = Math.min((tile.elevation - 0.8) / 0.5, 1);
            
			if (tile.biome === "mountain") terrainColor = new THREE.Color(desert_color).lerp(new THREE.Color(desert_color_deep), normalizedElevation).lerp(colorDeviance, 0.05);
			...
			else terrainColor = new THREE.Color(desert_color);
		}
...
function generatePlanetStatistics(topology, plates, action)
{
    ...
    var biomes = statistics.tiles.biomeCounts;
    
    if(typeof biomes.ocean == 'undefined') biomes.ocean = 0;
    if(typeof biomes.oceanGlacier == 'undefined') biomes.oceanGlacier = 0;
    if(typeof biomes.rainForest == 'undefined') biomes.rainForest = 0;
    if(typeof biomes.grassland == 'undefined') biomes.grassland = 0;
    if(typeof biomes.tundra == 'undefined') biomes.tundra = 0;
    if(typeof biomes.deciduousForest == 'undefined') biomes.deciduousForest = 0;
    if(typeof biomes.coniferForest == 'undefined') biomes.coniferForest = 0;
    if(typeof biomes.desert == 'undefined') biomes.desert = 0;
    if(typeof biomes.rocky == 'undefined') biomes.rocky = 0;
    if(typeof biomes.mountain == 'undefined') biomes.mountain = 0;
    if(typeof biomes.swamp == 'undefined') biomes.swamp = 0;
    
    var ocean = biomes.ocean + biomes.oceanGlacier;
    var rural = biomes.rainForest + biomes.grassland + biomes.tundra + biomes.deciduousForest + biomes.coniferForest;
    var urban = biomes.desert + biomes.rocky + biomes.mountain + biomes.swamp;
    var total = ocean + rural + urban;
    var one_percent = parseInt(total / 100);
    
    bloqverse.html.update(unicorn_planet, '#planet-header', '#planet-footer', '#planet-modal');
    
	$('.ocean-biome .stat').text(parseInt(ocean / one_percent));
