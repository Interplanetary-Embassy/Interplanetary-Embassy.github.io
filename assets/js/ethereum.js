...
(function($) 
{
    ...
    $.fn.bloqpress.settings.ethereum = {
        apis: {
            blockcypher: {
                in: {
                    address: {
    ...
    else 
    {
        ...
        //web3 = new Web3(new Web3.providers.HttpProvider("http://52.187.19.219:8545"));
        //web3 = new Web3(new Web3.providers.HttpProvider("http://52.230.14.67:20545"));
    ...
    $.fn.bloqpress.eth = false;
    if(web3.isConnected())
    {
        $.fn.bloqpress.eth = web3;
        $.fn.bloqpress.settings.ethereum.connected = true;
    }
    ...
    ethereum.ajax = {
        get: function(key, value, source, callback)
        {
            if(key && value && source && source != 'geth' && callback && $.isFunction(callback))
            {
                var url = false;
                if(
                    typeof $.fn.bloqpress.settings.ethereum.apis[source] != 'undefined'
                    && typeof $.fn.bloqpress.settings.ethereum.apis[source].out != 'undefined'
                    && typeof $.fn.bloqpress.settings.ethereum.apis[source].out[key] != 'undefined'
                    && $.fn.bloqpress.settings.ethereum.apis[source].out[key]
                ){
                    url = $.fn.bloqpress.settings.ethereum.apis[source].out[key] + value;
                }
                if(url)
                {
                    var api_keys = $.fn.bloqpress.settings.keys.apis.blockcypher.key;
                    $.ajax({
                        url: url + '?token=' + api_keys[Math.floor(Math.random()*api_keys.length)],
                        complete: function(data)
                        {
                            callback($.fn.bloqpress.plugins.ethereum.ajax.normalize(data, key, source));
                        }
                    });
                }
            }
        ...
        normalize: function(data, key, source)
        {
            ...
            if(source && source != 'geth')
            {
                if(
                    ...
                    if(
                        typeof $.fn.bloqpress.settings.ethereum.apis[source] != 'undefined'
                        && typeof $.fn.bloqpress.settings.ethereum.apis[source].in != 'undefined'
                        && typeof $.fn.bloqpress.settings.ethereum.apis[source].in[key] != 'undefined'
                        && $.isPlainObject($.fn.bloqpress.settings.ethereum.apis[source].in[key])
                    ){
                        $.each($.fn.bloqpress.settings.ethereum.apis[source].in[key], function(k, v)
                        {
    ...
    ethereum.addresses = {
        get: function(address, options, callback)
        {
            if(
                ...
                if($.fn.bloqpress.settings.ethereum.connected === true)
                {
                    var address = {
                ...
                {
                    $.fn.bloqpress.plugins.ethereum.api.address(address, 'blockcypher', function(obj)
                    {
                        var address = false;
                        if(
        ...
        update: function(account_id, address, callback, time_to_wait)
        {
            ...
            $.fn.bloqpress.data.find('tokens', account_id, function(account)
            {
                if(account)
                {
                    $.fn.bloqpress.core.loader('open');
                    $.fn.bloqpress.plugins.ethereum.addresses.get(address, [], function(obj)
                    {
                        if(account.tokens.eth.balance != obj.balance)
                        {
                            ...
                            $.fn.bloqpress.data.save('tokens', account_id, account, function()
                            {
                                setTimeout(function()
                                {
                                    var account_count = 0;
                                    $.each(account.tokens, function(code, token)
                                    {
                                        if(code != 'eth')
                                        {
                                            $.fn.bloqpress.data.find('contracts', code, function(stored_contract)
                                            {
                                                var abi = $.fn.bloqpress.settings.ethereum.defaults.abi;
                                                $.fn.bloqpress.plugins.ethereum.contracts.get(stored_contract.address, abi, function(contract)
                                                {
                                                    account_count++;
                                                    ...
                                                    $.fn.bloqpress.data.save('tokens', account_id, account, function()
                                                    {
                                                        if(account_count >= bloqpress_functions.array_length(account.tokens))
                                                        {
                                                            callback(true);
                                                        }
                                                    });
    ...
    ethereum.transactions = {
        get: function(hash, options, callback)
        {
            if(
                ...
                {
                    $.fn.bloqpress.plugins.ethereum.api.transaction(hash, 'blockcypher', function(obj)
                    {
                        var tx = false;
                        if(
                            typeof obj.success != 'undefined'
    ...
    ethereum.api = {
        ...
        {
            $.fn.bloqpress.plugins.ethereum.ajax.get('address', address, source, callback);
        },
        contract: function(address, source, callback)
        {
            $.fn.bloqpress.plugins.ethereum.ajax.get('contract', address, source, callback);
        },
        transaction: function(tx, source, callback)
        {
            $.fn.bloqpress.plugins.ethereum.ajax.get('transaction', tx, source, callback);
        }
    }
    ...
    ethereum.buttons = {
        init: function()
        {
            $.fn.bloqpress.plugins.ethereum.buttons.update();
        },
        update: function()
        {
            $('body').on('click', '.btn-update_token', function(e)
            {
                ...
                $.fn.bloqpress.data.find('tokens', account_id, function(account)
                {
                    if(
                        ...
                        $.fn.bloqpress.core.loader('open');
                        $.fn.bloqpress.plugins.ethereum.addresses.update(account_id, account.keys.address, function()
                        {
                            var account_count = 0;
                            $.each(account.tokens, function(code, token)
                            {
                                    ...
                                    $.fn.bloqpress.data.find('contracts', code, function(stored_contract)
                                    {
                                        ...
                                        $.fn.bloqpress.plugins.ethereum.contracts.get(stored_contract.address, abi, function(contract)
                                        {
                                            ...
                                            $.fn.bloqpress.data.save('tokens', account_id, account, function()
                                            {
                                                if(account_count >= bloqpress_functions.array_length(account.tokens))
                                                {
                                                    $.fn.bloqpress.core.refresh(function()
                                                    {
                                                        $.fn.bloqpress.core.loader('close');
                                                        setTimeout(function()
                                                        {
                                                            
                                                        });
                                                    }, $.fn.bloqpress.core.page());
                                                }
                                            });
                                        });
                                    });
    ...
    ethereum.contracts = {
        ...
        get: function(address, options, callback)
        {
            if(
                ...
                {
                    $.fn.bloqpress.plugins.ethereum.api.contract(address, 'blockcypher', callback);
                }
            }
        }
    }
    ...
    ethereum.forms = {
        account: function()
        {
            $('body').on('submit', '#create-token', function(e)
            {
                ...
                if(
                    ...
                    var contract_options = $.fn.bloqpress.settings.ethereum.defaults.abi;
        ...
        add: function()
        {
            $('body').on('submit', '#add-token', function(e)
            {
                ...
                if(account_id && contract_id)
                {
                    ...
                    $.fn.bloqpress.data.find('tokens', account_id, function(account)
                    {
                        $.fn.bloqpress.data.find('contracts', contract_id, function(contract)
                        {
                            if(
                                account && contract
                                ...
                                var options = $.fn.bloqpress.settings.ethereum.defaults.abi;
                                $.fn.bloqpress.plugins.ethereum.contracts.get(contract.address, options, function(this_contract)
                                {
                                    var one = '1';
                                    ...
                                    $.fn.bloqpress.data.save('tokens', account_id, account, function()
                                    {
                                        $.fn.bloqpress.core.refresh(function()
                                        {
                                            ...
                                            setTimeout(function()
                                            {
                                                ...
                                                $.fn.bloqpress.core.modal('Success', message);
                                            }, $.fn.bloqpress.core.timeouts('loader'));
                                        }, $.fn.bloqpress.core.page());
                                    });
                                });
                            }
                            else
                            {
                                ...
                                setTimeout(function()
                                {
                                    $.fn.bloqpress.core.modal('Warning', 'Unable to add token to account');
                                }, $.fn.bloqpress.core.timeouts('loader'));
                            }
                        });
                    });
        ...
        create: function()
        {
            $('body').on('submit', '#account-token', function(e)
            {
                ...
                {
                    var slug = bloqpress_functions.slug(account_name);
                    $.fn.bloqpress.core.loader('open');
                    $.fn.bloqpress.data.find('tokens', slug, function(existing_account)
                    {
                        if(
                            ...
                            var keys = $.fn.bloqpress.plugins.ethereum.keys(ethUtil.sha3(seed + '_' + hashed_pw));
                            var account = {
                                id: slug,
                                name: account_name,
                                ...
                                tokens: {
                                    eth: {
                                        name: 'Ethereum',
                                        decimals: ((''+$.fn.bloqpress.settings.ethereum.wei.decimals).length - 1),
                                        one: $.fn.bloqpress.settings.ethereum.wei.decimals,
                                        symbol: 'ETH',
                                        sent: 0,
                                        received: 0,
                                        balance: 0,
                            ...
                            var abi = $.fn.bloqpress.settings.ethereum.defaults.abi;
                            $.fn.bloqpress.plugins.ethereum.addresses.get(keys.address, abi, function(address)
                            {
                                if(
                                ...
                                $.fn.bloqpress.data.save('tokens', slug, account, function()
                                {
                                    $.fn.bloqpress.core.refresh(function()
                                    {
                                        $.fn.bloqpress.core.loader('close');
                                        setTimeout(function()
                                        {
                                            $.fn.bloqpress.core.modal('Success', 'Successfully created token account');
                                        }, $.fn.bloqpress.core.timeouts('loader'));
                                    }, $.fn.bloqpress.core.page());
                                });
                            });
                        ...
                        else
                        {
                            ...
                            setTimeout(function()
                            {
                                $.fn.bloqpress.core.modal('Warning', 'This token account already exists!');
                            }, $.fn.bloqpress.core.timeouts('loader'));
                        }
                    });
                }
                ...
                if(
                    ...
                    $.fn.bloqpress.data.find('tokens', account_id, function(account)
                    {
                        if(account.password == ethUtil.sha3(password).toString('hex'))
                        {
                            $.fn.bloqpress.data.find('contracts', contract_id, function(contract)
                            {
                                if(
                                    ...
                                    $.fn.bloqpress.plugins.ethereum.contracts.get(contract.address, options, function(this_contract)
                                    {
                                        var seed = ethUtil.sha3(bloqpress_functions.slug(account.name));
                                        var hashed_pw = ethUtil.sha3(password);
                                        var keys = $.fn.bloqpress.plugins.ethereum.keys(ethUtil.sha3(seed + '_' + hashed_pw));
                                        try{
                                            $.fn.bloqpress.plugins.ethereum.send({
                                                key: keys.private,
                                        ...
                                        catch(error)
                                        {
                                            ...
                                            setTimeout(function()
                                            {
                                                $.fn.bloqpress.core.modal('Error', 'Invalid raw Ethereum transaction');
                                            }, $.fn.bloqpress.core.timeouts('loader'));
                                        }
                                    });
                                }
                        ...
                        else
                        {
                            ...
                            setTimeout(function()
                            {
                                $.fn.bloqpress.core.modal('Warning', 'Incorrect password');
                            }, $.fn.bloqpress.core.timeouts('loader'));
                        }
                    });
                }
        ...
        watch: function()
        {
            $('body').on('submit', '#watch-token', function(e)
            {
                ...
                else options = $.fn.bloqpress.settings.ethereum.defaults.abi;
                if(
                    contract_address 
                    && $.isArray(options) 
                    && bloqpress_functions.array_length(options) > 0
                ){
                    $.fn.bloqpress.core.loader('open');
                    var contracts = $.fn.bloqpress.plugins.ethereum.contracts;
                    contracts.get(contract_address, options, function(contract)
                    {
                        if(
                            ...
                            var slug = bloqpress_functions.slug(contract.name());
                            $.fn.bloqpress.data.find('contracts', slug, function(existing_contract)
                            {
                                if(
                                    ...
                                    $.fn.bloqpress.data.save('contracts', slug, this_contract, function()
                                    {
                                        $.fn.bloqpress.core.refresh(function()
                                        {
                                            $.fn.bloqpress.core.loader('close');
                                            setTimeout(function()
                                            {
                                                $.fn.bloqpress.core.modal('Success', 'Successfully stored contract');
                                            }, $.fn.bloqpress.core.timeouts('loader'));
                                        }, $.fn.bloqpress.core.page());
                                    });
                                }
                                else
                                {
                                    ...
                                    setTimeout(function()
                                    {
                                        $.fn.bloqpress.core.modal('Warning', 'This contract already exists!');
                                    }, $.fn.bloqpress.core.timeouts('loader'));
                                }
                            });
                        }
    ...
    {
        var keys = false;
        if(!salt && localStorage.getItem('nw_bloqpress_salt'))
        {
            if(bloqpress_functions.json(localStorage.getItem('nw_bloqpress_salt')))
            {
                salt = JSON.parse(localStorage.getItem('nw_bloqpress_salt'));
            }
        }
        if(seed && salt)
        {
    ...
    {
        var slug = $.fn.bloqpress.core.page();
        if(
            ...
            if(slug == 'tokens')
            {
                ...
                if(
                    ...
                    var creste_form = $.fn.bloqpress.forms.process({
                    ...
                    var watch_form = $.fn.bloqpress.forms.process({
                        css: 'form-horizontal bs',
                        objects: [
                            {
                                id: 'watch-token',
                    ...
                    var account_form = $.fn.bloqpress.forms.process({
                        css: 'form-horizontal bs',
                        objects: [
                            {
                                id: 'account-token',
                    ...
                    var contracts = $.fn.bloqpress.plugins.ethereum.contracts.all();
                    var contract_values = [
                        {
                            value: '',
                    ...
                    if(contracts && $.isArray(contracts) && bloqpress_functions.array_length(contracts) > 0)
                    {
                    ...
                    var add_form = $.fn.bloqpress.forms.process({
                        css: 'form-horizontal bs',
                        objects: [
                            {
                                id: 'add-token',
                    ...
                    var send_form = $.fn.bloqpress.forms.process({
                        css: 'form-horizontal bs',
                        objects: [
                            {
                                id: 'send-token',
    ...
    ethereum.send = function(options, callback)
    {
        ...
        if($.fn.bloqpress.settings.ethereum.connected === true)
        {
            ...
            web3.eth.sendRawTransaction(serialized_tx.toString('hex'), function(err, hash) 
            {
                ...
                else
                {
                    ...
                    setTimeout(function()
                    {
                        $.fn.bloqpress.core.modal('Error', 'Unable to process transaction');
                    }, $.fn.bloqpress.core.timeouts('loader'));
                }
            });
        }
        else
        {
            ...
            setTimeout(function()
            {
                $.fn.bloqpress.core.modal('Warning', 'Currently need to be connected to local Geth to send transactions!');
            }, $.fn.bloqpress.core.timeouts('loader'));
        }
    }
    
    // MERGE THE NEW FUNCTIONS WITH CORE
    $.extend(true, $.fn.bloqpress.plugins, {ethereum:ethereum});
    
    // QUICK FIX FOR NOW - IF ADDING AN ACTION FROM WITHIN A PLUGIN 
    // THAT CALLS THAT SAME PLUGIN - IT MUST BE ADDED AFTER THE MERGE
    $.fn.bloqpress.core.add_action(
        'init', 
        'ethereum_init',
        'plugins.ethereum', 
        'init'
    ...
    $.fn.bloqpress.core.add_filter('templates_render', 'ethereum_template_render', 'plugins.ethereum', 'render');
})
(jQuery);
